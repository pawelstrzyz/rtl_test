 ...
 */	
	function SEO_URL_INSTALLER(){
		...
		$this->default_config['USE_SEO_CACHE_INFO_PAGES'] = array('DEFAULT' => 'true',
		   														  'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES ('', 'Enable Page Manager cache?', 'USE_SEO_CACHE_INFO_PAGES', 'true', 'This will turn off caching for the information pages.', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')"
																  );
		$x++;
 ...
 */	
	function generate_categories_cache(){
		...
		if ( !$is_cached || $is_expired ) { // it's not cached so create it
			switch(true){
				case ($this->attributes['SEO_ADD_CAT_PARENT'] == 'true'):
					$sql = "SELECT c.categories_id as id, c.parent_id, cd.categories_name as cName, cd2.categories_name as pName, cd.categories_seo_url as csu, cd2.categories_seo_url as psu  
							FROM ".TABLE_CATEGORIES." c
...
if($category['csu'] != '')
			{
				if($this->not_null($category['psu'])){
					$name = $category['psu'] . ' ' . $category['csu'];
				} else if($this->not_null($category['pName'])){
			  		$name = $category['pName'] . ' ' . $category['csu'];
				} else {
					$name = $category['csu'];
				}
			}
