<?php
/*
  $Id: cache.php 135 2008-02-27 21:26:23Z  $
...
// Cache the manufacturers box
  ...
  function tep_cache_manufacturers_std_box($auto_expire = false, $refresh = false) {
    global $HTTP_GET_VARS, $language;
    ...
    if (($refresh == true) || !read_cache($cache_output, 'manufacturers_std_box-' . $language . '.cache' . $manufacturers_id, $auto_expire)) {
      ob_start();
      include(DIR_WS_BOXES . 'manufacturers_std.php');
      $cache_output = ob_get_contents();
      ob_end_clean();
      write_cache($cache_output, 'manufacturers_std_box-' . $language . '.cache' . $manufacturers_id);
    }
...
// Cache the also purchased module
  function tep_cache_also_purchased($auto_expire = false, $refresh = false) {
    global $HTTP_GET_VARS, $language, $languages_id, $currencies;
  ...
  ////
  // Cache the also best_sellers module
  function tep_cache_best_sellers_box($auto_expire = false, $refresh = false) {
    global $language, $languages_id, $current_category_id;
    ...
    if (($refresh == true) || !read_cache($cache_output, 'best_sellers_box-' . $language . '.cache' . $current_category_id, $auto_expire)) {
      ob_start();
      include(DIR_WS_BOXES . 'best_sellers.php');
      $cache_output = ob_get_contents();
      ob_end_clean();
      write_cache($cache_output, 'best_sellers_box-' . $language . '.cache' . $current_category_id);
    }
