...
SART 31.08.2005
 ...
 $Id: pocztapriorytetowa.php 1 2007-12-20 23:52:06Z kamelianet $
 ...
 easily changed by editing the line below in the pocztapriorytetowa constructor
 that defines $this->num_pocztapriorytetowa.
 ...
 pocztapriorytetowa.php.  A list of settings will appear to the right.  Click on the
 Edit button.
...
// class methods
   ...
   function install() {
     ...
     //tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Koszt Pobrania', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_KOSZT_POBRANIA', '6', 'Dodatkowy koszt wysyłki gdy za pobraniem (tylko obszar Polski).', '6', '0', now())");
     for ($i = 1; $i <= $this->num_pocztapriorytetowa; $i++) {
        $default_countries = '';
       if ($i == 1) {        
         $strefa = 'Polska';
       ...
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Kraje Obszaru Nr " . $i ." (strefa " . $strefa . " wg cennika Poczty Polskiej)', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Oddzielona przecinkami lista dwuznakowych symboli ISO w ramach obszaru Nr " . $i . ".', '6', '0', now())");
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Tablica kosztów Nr " . $i ." (strefa " . $strefa . " wg cennika Poczty Polskiej)', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_COST_" . $i ."', '" . $shipping_table . "', 'Koszty przesyłki dla Obszaru Nr " . $i . " bazujące na maksymalnej wadze sumarycznej zamowienia. Przykład: 3:8.50,7:10.50,... zamowienia do wagi 3 kosztują 8.50, do wagi 7 kosztują 10.50 w obszarze Nr " . $i . ".', '6', '0', now())");
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Dodatkowy stały koszt obsługi wysyłki do Obszaru Nr " . $i ."', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_HANDLING_" . $i ."', '0', 'Dodatkowy stały koszt obsługi wysyłki do tego Obszaru ', '6', '0', now())");
     }
   }
   ...
   function keys() {
     //$keys = array('MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_STATUS', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_TAX_CLASS', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_SORT_ORDER','MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_KOSZT_POBRANIA');
     $keys = array('MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_STATUS', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_TAX_CLASS', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_SORT_ORDER');
