...
SART 31.08.2005
 ...
 $Id: pocztapripobraniowa.php 1 2007-12-20 23:52:06Z $
 ...
 easily changed by editing the line below in the pocztapripobraniowa constructor
 that defines $this->num_pocztapripobraniowa.
 ...
 pocztapripobraniowa.php.  A list of settings will appear to the right.  Click on the
 Edit button.
...
*/
 ...
 class pocztapripobraniowa {
   var $code, $title, $description, $enabled, $num_pocztapripobraniowa;

// class constructor
   function pocztapripobraniowa() {
	   global $order;
     $this->code = 'pocztapripobraniowa';
     $this->title = MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_TEXT_TITLE;
     $this->description = MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_TEXT_DESCRIPTION;
     $this->sort_order = MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_SORT_ORDER;
     $this->icon = '';
     $this->tax_class = MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_TAX_CLASS;
     $this->enabled = ((MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_STATUS == 'True') ? true : false);
     ...
     $this->num_pocztapripobraniowa = 1;
   }
...
// class methods
   function quote($method = '') {
     ...
     for ($i=1; $i<=$this->num_pocztapripobraniowa; $i++) {
       $countries_table = str_replace(' ','',constant('MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_COUNTRIES_' . $i));
       $country_pocztapripobraniowa = split("[;]", $countries_table);
       if (in_array($dest_country, $country_pocztapripobraniowa)) {
         $dest_zone = $i;
         break;
       }
     }
     ...
     if ($dest_zone == 0) {
       $dest_zone = $this->num_pocztapripobraniowa;    // the zone is the lastest zone avalaible
     }
     // elari - Added to select default country if not in listing
     if ($dest_zone == 0) {
       ...
       $shipping = -1;
       $pocztapripobraniowa_cost = constant('MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_COST_' . $dest_zone);
       $pocztapripobraniowa_table = split("[:,]" , $pocztapripobraniowa_cost);
       $size = sizeof($pocztapripobraniowa_table);

       for ($i=0; $i<$size; $i+=2) {
         if ($shipping_weight <= $pocztapripobraniowa_table[$i]) {
           $shipping = $pocztapripobraniowa_table[$i+1];
           $shipping_method = MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_TEXT_WAY . ' ' . $order->delivery['country']['title'] . ': ';
           if ($shipping_num_boxes > 1) {
             $shipping_method .= $shipping_num_boxes . 'x ';
           }
           $shipping_method .= $shipping_weight . ' ' . MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_TEXT_UNITS;
           break;
         }
       }
       ...
       if ($shipping == -1) {
         $shipping_cost = 0;
         $shipping_method = MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_UNDEFINED_RATE;
       } else {
         $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_HANDLING_' . $dest_zone);
       }
     }

     $this->quotes = array('id' => $this->code,
                           'module' => MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_TEXT_TITLE,
     ...
     if ($error == true) $this->quotes['error'] = MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_INVALID_ZONE;
     return $this->quotes;
   }

   function check() {
     if (!isset($this->_check)) {
       $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_STATUS'");
   ...
   // elari - Added to select default country if not in listing
   function install() {
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Włączenie modułu', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_STATUS', 'True', 'Czy włączyć moduł Przesyłka Priorytetowa Pobraniowa ?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Grupa VAT', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_TAX_CLASS', '0', 'Użyj następującej grupy VAT odnośnie tego kosztu wysyłki.', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sortowanie', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_SORT_ORDER', '0', 'Kolejność wyświetlania wśród innych modułów wysyłki.', '6', '0', now())");
     //tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Koszt Pobrania', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_KOSZT_POBRANIA', '6', 'Dodatkowy koszt wysyłki gdy za pobraniem (tylko obszar Polski).', '6', '0', now())");
     for ($i = 1; $i <= $this->num_pocztapripobraniowa; $i++) {
        $default_countries = '';
       if ($i == 1) {        
      $default_countries = 'PL'; ///przesylka priorytetowa krajowa pobraniowa
         $shipping_table = '0.5:9,1:11,2:12.5,5:14,10:21,15:27,20:35,30:40'; 
       }
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Kraje Obszaru Nr " . $i ."', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Oddzielona przecinkami lista dwuznakowych symboli ISO w ramach obszaru Nr " . $i . ".', '6', '0', now())");
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Tablica kosztów Obszaru Nr " . $i ."', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_COST_" . $i ."', '" . $shipping_table . "', 'Koszty przesyłki dla Obszaru Nr " . $i . " bazujące na maksymalnej wadze sumarycznej zamowienia. Przykład: 3:8.50,7:10.50,... zamowienia do wagi 3 kosztują 8.50, do wagi 7 kosztują 10.50 w obszarze Nr " . $i . ".', '6', '0', now())");
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Dodatkowy stały koszt obsługi wysyłki do Obszaru Nr " . $i ."', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_HANDLING_" . $i ."', '0', 'Dodatkowy stały koszt obsługi wysyłki do tego Obszaru ', '6', '0', now())");
     }
   }
   ...
   function keys() {
    //$keys = array('MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_STATUS', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_TAX_CLASS', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_SORT_ORDER','MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_KOSZT_POBRANIA');
    $keys = array('MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_STATUS', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_TAX_CLASS', 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_SORT_ORDER');
     for ($i=1; $i<=$this->num_pocztapripobraniowa; $i++) {
       $keys[] = 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_COUNTRIES_' . $i;
       $keys[] = 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_COST_' . $i;
       $keys[] = 'MODULE_SHIPPING_PRZESYLKA_PRIORYTETOWA_POB_HANDLING_' . $i;
     }
     return $keys;
   }
 }
