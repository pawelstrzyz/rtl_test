<?php
/*
  $Id: admin_account.php 1 2007-12-20 23:52:06Z $
...
//link_post_variable('check_password');
//link_post_variable('check_account');
    ...
    switch ($HTTP_GET_VARS['action']) {
      case 'check_password':
        ...
        if (!tep_validate_password($HTTP_POST_VARS['password_confirmation'], $check_pass['confirm_password'])) {
          tep_redirect(tep_href_link(FILENAME_ADMIN_ACCOUNT, 'action=check_account&error=password'));
  		    ...
  		    /// iSolve addition 3rd Oct 2006 START
      ...
      case 'save_account':
        ...
        if (in_array($HTTP_POST_VARS['admin_email_address'], $stored_email)) {
          ...
          $temat = MakeUTF(ADMIN_EMAIL_SUBJECT);
          tep_mail($HTTP_POST_VARS['admin_firstname'] . ' ' . $HTTP_POST_VARS['admin_lastname'], $HTTP_POST_VARS['admin_email_address'], $temat, sprintf(ADMIN_EMAIL_TEXT, $HTTP_POST_VARS['admin_firstname'], HTTP_SERVER . DIR_WS_ADMIN, $HTTP_POST_VARS['admin_email_address'], $hiddenPassword, STORE_OWNER), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
...
//    if ( ($HTTP_GET_VARS['action'] == 'edit_process') && (tep_session_is_registered('confirm_account')) ) {
    if ( ($HTTP_GET_VARS['action'] == 'edit_process') && (array_key_exists('confirm_account', $_SESSION)) ) {
?>
                    <tr>
...
?>                       
              ...
              <tr>
                <td><table width="100%" border="0" cellspacing="0" cellpadding="3"><tr><td class="smallText" valign="top"><?php echo TEXT_INFO_MODIFIED . $myAccount['admin_modified']; ?></td><td align="right">
                <?php 
                if ($HTTP_GET_VARS['action'] == 'edit_process') { 
                   echo '<a href="' . tep_href_link(FILENAME_ADMIN_ACCOUNT) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a> '; 
                   if (array_key_exists('confirm_account', $_SESSION)) { 
                     echo tep_image_submit('button_save.gif', IMAGE_SAVE, 'onClick="validateForm();return document.returnValue"'); 
                   } 
