<?php
/*
  $Id: admin_members_edit.php 76 2008-02-16 09:45:20Z jmk $
    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      case 'group_define':
        ...
        while ($define_files = tep_db_fetch_array($define_files_query)) {
          ...
          if (in_array ($admin_files_id, $selected_checkbox)) {
            $sql_data_array = array('admin_id' => tep_db_prepare_input($HTTP_POST_VARS['checked_' . $admin_files_id]));
            //$set_group_id = $HTTP_POST_VARS['checked_' . $admin_files_id];
          } else {
            $sql_data_array = array('admin_id' => tep_db_prepare_input($HTTP_POST_VARS['unchecked_' . $admin_files_id]));
            //$set_group_id = $HTTP_POST_VARS['unchecked_' . $admin_files_id];
          }
		  //$sql_data_array = array('admin_access_values' => tep_db_prepare_input($HTTP_POST_VARS[ $admin_access_values]));
          tep_db_perform(TABLE_ADMIN_FILES, $sql_data_array, 'update', 'admin_files_id = \'' . $admin_files_id . '\'');

        }

        tep_redirect(tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gID=' . $HTTP_POST_VARS['admin_id']));
        break;
      case 'group_delete':
        $set_admin_id = tep_db_prepare_input($HTTP_POST_VARS['set_admin_id']);

        tep_db_query("delete from " . TABLE_ADMIN . " where admin_id = '" . $HTTP_GET_VARS['gID'] . "'");
        ...
        break;
      case 'group_edit':
        $admin_firstname = ucwords(strtolower(tep_db_prepare_input($HTTP_POST_VARS['admin_firstname'])));
        $name_replace = ereg_replace (" ", "%", $admin_firstname);

        if (($admin_firstname == '' || NULL) || (strlen($admin_firstname) <= 3) ) {
          tep_redirect(tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gID=' . $HTTP_GET_VARS[gID] . '&gName=false&action=action=edit_group'));
        } else {
          $check_groups_name_query = tep_db_query("select admin_firstname as group_name_edit from " . TABLE_ADMIN . " where admin_id <> " . $HTTP_GET_VARS['gID'] . " and admin_firstname like '%" . $name_replace . "%'");
          $check_duplicate = tep_db_num_rows($check_groups_name_query);
          if ($check_duplicate > 0){
            tep_redirect(tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gID=' . $HTTP_GET_VARS['gID'] . '&gName=used&action=edit_group'));
          } else {
            $admin_id = $HTTP_GET_VARS['gID'];
      ...
      case 'group_new':
        ...
        if (($admin_firstname == '' || NULL) || (strlen($admin_firstname) <= 1) ) {
          tep_redirect(tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gID=' . $HTTP_GET_VARS[gID] . '&gName=false&action=new_group'));
        } else {
          ...
          $check_duplicate = tep_db_num_rows($check_groups_name_query);
          if ($check_duplicate > 0){
            tep_redirect(tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gID=' . $HTTP_GET_VARS['gID'] . '&gName=used&action=new_group'));
          } else {
		...
		$admin_access_values = tep_db_prepare_input($HTTP_POST_VARS['admin_access_values']);   
		...
		case 'update_access':
		$gPath = tep_db_prepare_input($HTTP_POST_VARS['gPath']);
	$file_access_id = tep_db_prepare_input($HTTP_POST_VARS['file_access_id']);
		$admin_access_values = tep_db_prepare_input($HTTP_POST_VARS['admin_access_values']); 
		$delete_value = tep_db_prepare_input($HTTP_POST_VARS['delete_value']);  
			if ($delete_value != '1') {
...
<?php
 if ($HTTP_GET_VARS['gPath']) {
   $group_name_query = tep_db_query("select admin_firstname, admin_lastname from " . TABLE_ADMIN . " where admin_id = " . $HTTP_GET_VARS['gPath']);
   $group_name = tep_db_fetch_array($group_name_query);

   if ($HTTP_GET_VARS['gPath'] == 1) {
     echo tep_draw_form('defineForm', FILENAME_ADMIN_MEMBERS_EDIT, 'gID=' . $HTTP_GET_VARS['gPath']);
   } elseif ($HTTP_GET_VARS['gPath'] != 1) {
     echo tep_draw_form('defineForm', FILENAME_ADMIN_MEMBERS_EDIT, 'gID=' . $HTTP_GET_VARS['gPath'] . '&action=group_define', 'post', 'enctype="multipart/form-data"');
     echo tep_draw_hidden_field('admin_id', $HTTP_GET_VARS['gPath']);
   }
?>
...
?>
					  ...
					  <td width="441" class="dataTableContent"> <div align="right">
                              <?php 
			$admin_access_query = tep_db_query("select file_access_id, admin_files_id, admin_id from " .TABLE_ADMIN_ACCESS_FILES . " where admin_id='" . $HTTP_GET_VARS['gPath'] . "' and admin_files_id='" . $group_boxes_files['admin_files_id'] . "'");
...
if ($HTTP_GET_VARS['gPath'] != 1 and $admin_files_id >0) { echo  '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gPath=' . $HTTP_GET_VARS['gPath'] . '&file_access_id='.$admin_access['file_access_id'].'&action=edit_access').'">','<font color="#0066cc">' .TEXT_EDIT_ACCESS_VALUE . '</font></a>&nbsp;';
} 
elseif ($admin_files_id=="" and $HTTP_GET_VARS['gPath'] != 1) {
echo '&nbsp;<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gPath=' . $HTTP_GET_VARS['gPath'] . '&admin_files_id='.$group_boxes_files['admin_files_id'].'&action=insert_access').'">', '<font color="#FF0000">' .TEXT_ADD_ACCESS_VALUE . '</font></a> '; } 
...
?>
              <tr class="dataTableRowBoxes">
                <td colspan=2 class="dataTableContent" valign="top" align="right"><?php if ($HTTP_GET_VARS['gPath'] != 1) { echo  '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a> ' . tep_image_submit('button_save.gif', IMAGE_INSERT); } else { echo tep_image_submit('button_back.gif', IMAGE_BACK); } ?>&nbsp;</td>
              </tr>
            </table></form>
<?php
...
<?php
  ...
  while ($groups = tep_db_fetch_array($db_groups_query)) {
    ...
    if (((!$HTTP_GET_VARS['gID']) || ($HTTP_GET_VARS['gID'] == $groups['admin_id']) || ($HTTP_GET_VARS['gID'] == 'groups')) && (!$gInfo) ) {
...
?>
                ...
                <td class="<?php echo $current_row; ?>" align="right"><?php if ( (is_object($gInfo)) && ($groups['admin_id'] == $gInfo->admin_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gID=' . $groups['admin_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    $count_groups++;
  }
              ...
              <tr>
                <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td class="smallText" valign="top" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a> <a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gID=' . $gInfo->admin_id . '&action=new_group') . '">' . tep_image_button('button_admin_group.gif', IMAGE_NEW_GROUP) . '</a>'; ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table>
...
?>
            <table border="0" width="100%" cellspacing="0" cellpadding="2" style="border-color: #424242; border-width: 1px; border-style: solid;">
              <tr class="dataTableHeadingRow">
...
//Mett
  while ($admin = tep_db_fetch_array($db_admin_query)) {
    ...
    if ( (is_object($mInfo)) && ($admin['admin_id'] == $mInfo->admin_id) ) {
      echo '                  <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'page=' . $HTTP_GET_VARS['page'] . '&mID=' . $admin['admin_id'] . '&action=edit_member') . '\'">' . "\n";
      $current_row='dataTableContentSelectedList';
    } else {
      echo '                  <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'page=' . $HTTP_GET_VARS['page'] . '&mID=' . $admin['admin_id']) . '\'">' . "\n";
      $current_row='dataTableContent';
    }
?>
                ...
                <td class="<?php echo $current_row; ?>" align="right"><?php if ( (is_object($mInfo)) && ($admin['admin_id'] == $mInfo->admin_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'page=' . $HTTP_GET_VARS['page'] . '&mID=' . $admin['admin_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($HTTP_GET_VARS['action']) {
    ...
    case 'new_group':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gPath=' . $HTTP_GET_VARS['gPath']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a> ' . tep_image_submit('button_next.gif', IMAGE_NEXT) );
      break;
    case 'edit_group':
      $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_EDIT_GROUP . '</b>');

      $contents = array('form' => tep_draw_form('edit_group', FILENAME_ADMIN_MEMBERS_EDIT, 'action=group_edit&gID=' . $HTTP_GET_VARS['gID'], 'post', 'enctype="multipart/form-data"'));
      if ($HTTP_GET_VARS['gName'] == 'false') {
        $contents[] = array('text' => TEXT_INFO_GROUPS_NAME_FALSE . '<br>&nbsp;');
      } elseif ($HTTP_GET_VARS['gName'] == 'used') {
        $contents[] = array('text' => TEXT_INFO_GROUPS_NAME_USED . '<br>&nbsp;');
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.gif', IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gPath=' . $HTTP_GET_VARS['gPath']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    case 'del_group':
      ...
      if ($gInfo->admin_id == 1) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gPath=' . $HTTP_GET_VARS['gPath']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a><br>&nbsp;');
      } else {
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gPath=' . $HTTP_GET_VARS['gPath']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a><br>&nbsp;');
      }
      break;
    case 'define_group':
      ...
      if ($HTTP_GET_VARS['gPath'] == 1) {
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT) . '">' . tep_image_button('button_back.gif', IMAGE_CANCEL) . '</a><br>');
      }
      break;
	
	  case 'insert_access':
  $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_DEFINE . '</b>');  
		   $admin_access_query = tep_db_query("select admin_files_id, admin_files_name, admin_files_is_boxes from " .TABLE_ADMIN_FILES . " where admin_files_id=".$HTTP_GET_VARS['admin_files_id']." and admin_files_is_boxes = '0'");
$admin_access = tep_db_fetch_array($admin_access_query);
$admin_id= $HTTP_GET_VARS['gPath'];
$admin_files_id = $admin_access['admin_files_id']; 
 ...
 $contents[] = array('text' => sprintf($admin_access['admin_files_name']));
	...
	$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.gif', IMAGE_SAVE, 'onClick="validateForm();return document.returnValue"') . ' <a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'page=' . $HTTP_GET_VARS['page'] . '&mID=' . $HTTP_GET_VARS['mID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
	  echo '</form>';
	  break;
...
while($admin_edit = tep_db_fetch_array($admin_edit_query)){
 $contents[] = array('text' => sprintf($admin_edit['admin_files_name']));
	...
	//delete -reset
	 ...
	 $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.gif', IMAGE_SAVE, 'onClick="validateForm();return document.returnValue"') . ' <a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
	  echo '</form>';
    break;
   ...
   case 'show_group':
    ...
    default:
      if (is_object($mInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gPath=' . $mInfo->admin_id . '&action=define_group') . '">' . tep_image_button('button_admin_permission.gif', IMAGE_FILE_PERMISSION) . '</a>');
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS_EDIT, 'gPath=' . $gInfo->admin_id . '&action=define_group') . '">' . tep_image_button('button_admin_permission.gif', IMAGE_FILE_PERMISSION) . '</a> ');
