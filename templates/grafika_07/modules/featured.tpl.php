...
while ($featured_products = tep_db_fetch_array($featured_products_query)) {
    ...
    if ($new_price = tep_get_products_special_price($featured_products['products_id']) ) {
        ...
        if ($featured_products['products_price'] > 0) {
            if ($special_hide == 'false') {
                ...
                if ((($query_price_to_guest_result=='true') && !(tep_session_is_registered('customer_id'))) || ((tep_session_is_registered('customer_id')))) {
                   $cena_produktu = '<span class="smallText">'. TABLE_HEADING_PRICE . ':</span>&nbsp;<span class="smallText"><s>' . $currencies->display_price($featured_products['products_id'], $featured_products_org['products_price'], tep_get_tax_rate($featured_products['products_tax_class_id'])) . '</s></span><br><span class="productSpecialPrice">' . $currencies->display_price_nodiscount($featured_products['products_price'],tep_get_tax_rate($featured_products['products_tax_class_id']),1,$display_nb) .'</span>';
                } else {
                   $cena_produktu = '<span class="smallText">'. TABLE_HEADING_PRICE . ':</span><br><span class="Cena">' . PRICES_LOGGED_IN_TEXT . '</span>';                }
            } else {
                $cena_produktu = '<span class="Cena">' . $currencies->display_price($featured_products['products_id'], $featured_products_org['products_price'], tep_get_tax_rate($featured_products['products_tax_class_id']),1,$display_nb) .'</span>';
            }
        } else {
        ...
        if ($featured_products['products_price'] > 0) {
            $cena_produktu = '<span class="smallText">'. TABLE_HEADING_PRICE . ':<br></span><span class="Cena">' . $currencies->display_price($featured_products['products_id'],$featured_products['products_price'],tep_get_tax_rate($featured_products['products_tax_class_id']),1,$display_nb).'</span>';
        } else {
            $cena_produktu = '<span class="smallText"><b>'.TEMPORARY_NO_PRICE.'</b></span>';
        }
    }
